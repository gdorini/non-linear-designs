...
ajusta.expr.fp <- function(p.lin, p, d.mod, a, b){
  aux <- c("a")
  aux <- c(p.lin, aux)
  l.a <- length(aux)
  aux <- ysym(aux)
  ...
  options(warn = 0)
  d.mod <- do.call(y_eval, c(list(d.mod), list.a))
  d.mod.2 <- y_eval(d.mod, "x^a"="Ln(x)", "x^(2*a)"="Ln(x)^2", "x0^a"="Ln(x0)", "x^(2*a)"="Ln(x)^2", "xm^a"="Ln(xm)", "xm^(2*a)"="(Ln(xm))^2") #para a=0
  d.mod <- y_eval(d.mod, x0=a, xm=b)
  d.mod.2 <- y_eval(d.mod.2, x0=a, xm=b)
  return(list(d.mod, d.mod.2))
}
...
integra_crit_v.fp <- function(integrando, low, up, X, W, alpha){
  
  func_int_v <- function(par, x, w, a){
    matrix(apply(par, 2, function(p){return(eval(parse(text=integrando)))}), ncol = ncol(par))
  }
  
  h_int_v <- hcubature(func_int_v, low, up, x = X, w = W, a = alpha, tol=1e-4, maxEval = 1000, vectorInterface = TRUE)
  return(h_int_v)
}
...
#realiza a integral por metodo de monte carlo ppara fp
integral_MC_IS.fp <- function(integrando, dominio, x, w, alpha){
  f_int.mc <- function(p, x, w, a){
    return(eval(parse(text=integrando)))
  }
  ...
  if(is.matrix(dominio)==TRUE){
    N <- nrow(dominio)
    for(j in 1:N){
      f_dominio[j] <- f_int.mc(dominio[j,], x, w, alpha)
    }}
  else{
    N <- length(dominio)
    for(j in 1:N){
